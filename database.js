...
//Commented out due to a mistake somewhere in the closing template strings. See new code below
...
export async function filterByQueryAndCategories(query, activeCategories) {
  // ========================== Code to test =================================
  let filterQuery = activeCategories
    .map((elem) => `category = '${elem}'`)
    .join(' or ');
  ...
  return new Promise((resolve, reject) => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        'select * from menuitems where title like ?' + filterQuery,
        ['%' + query + '%'],
        (_, { rows }) => {
          resolve(rows._array);
        }
